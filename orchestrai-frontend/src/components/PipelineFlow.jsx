...
function PipelineFlow({ steps }) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm dark:border-slate-800 dark:bg-slate-900" aria-label="Pipeline progress">
      <div className="mb-5 flex items-center justify-between gap-3">
        <div>
          <h2 className="text-lg font-semibold text-slate-950 dark:text-white">Pipeline Progress</h2>
      ...
      <ol className="grid gap-3 md:grid-cols-5">
        {PIPELINE_STEPS.map((step, index) => {
          ...
          return (
            <li key={step.key} className="relative rounded-lg border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-950">
...
function StatusIcon({ status }) {
  if (status === 'done') {
    return <span className="flex h-7 w-7 items-center justify-center rounded-full bg-emerald-100 text-sm font-bold text-emerald-700 dark:bg-emerald-950 dark:text-emerald-300">✓</span>
  }
  if (status === 'error') {
    return <span className="flex h-7 w-7 items-center justify-center rounded-full bg-rose-100 text-sm font-bold text-rose-700 dark:bg-rose-950 dark:text-rose-300">!</span>
  }
  if (status === 'running') {
    return (
